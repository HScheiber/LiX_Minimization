...
JobType = 'Structure_Minimization'; % 'Structure_Minimization', or 'Generate_GROMACS_PES'
...
Scale_Dispersion = 0.3:0.1:5.0; %[0.1:0.1:0.9 1.1:0.1:5.0]; % Works for both JC and TF
...
% Activate GLOST?
Glost_On = true; % activate GLOST: Greedy Launcher Of Small Tasks
...
elseif strcmp(Server,'sea') % Orcinus
...
elseif strcmp(Server,'ced') || strcmp(Server,'cdr') || strcmp(Server,'gra')
    TemplateText = ['#!/bin/bash' newline ... 
...
if strcmp(JobType,'Structure_Minimization')
    if Parallel
        error('Parallel mode not currently possible for Structure minimization jobs')
    elseif Glost_On
        if Optimize_Position
