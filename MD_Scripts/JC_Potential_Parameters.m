...
function [OutputMet,OutputHal] = JC_Potential_Parameters(Metal,Halide,Watermodel,plotswitch,Scaling_Params)
...
% Combine scaling factors into epsilon and sigma scaling factors
Gamma = S_E*(S_D^2)*(1/S_R); % Epsilon scaling factor
Beta = S_S*(1/(S_D^(1/6)))*(S_R^(1/6)); % Sigma scaling factor

%% JC Ion Parameters in SPC/E water
if strcmp(Watermodel,'SPC/E')
    Param.Li.sigma = Beta*(0.791*nm_per_Ang)*(2^(5/6)); % nm
    Param.Li.epsilon = Gamma*(0.3367344)*kJ_per_kcal; % kJ/mol

    Param.Na.sigma = Beta*(1.212*nm_per_Ang)*(2^(5/6)); % nm
    Param.Na.epsilon = Gamma*(0.3526418)*kJ_per_kcal; % kJ/mol

    Param.K.sigma = Beta*(1.593*nm_per_Ang)*(2^(5/6)); % nm
    Param.K.epsilon = Gamma*(0.4297054)*kJ_per_kcal; % kJ/mol

    Param.Rb.sigma = Beta*(1.737*nm_per_Ang)*(2^(5/6)); % nm
    Param.Rb.epsilon = Gamma*(0.4451036)*kJ_per_kcal; % kJ/mol

    Param.Cs.sigma = Beta*(2.021*nm_per_Ang)*(2^(5/6)); % nm
    Param.Cs.epsilon = Gamma*(0.0898565)*kJ_per_kcal; % kJ/mol

    Param.F.sigma = Beta*(2.257*nm_per_Ang)*(2^(5/6)); % nm
    Param.F.epsilon = Gamma*(0.0074005)*kJ_per_kcal; % kJ/mol

    Param.Cl.sigma = Beta*(2.711*nm_per_Ang)*(2^(5/6)); % nm
    Param.Cl.epsilon = Gamma*(0.0127850)*kJ_per_kcal; % kJ/mol

    Param.Br.sigma = Beta*(2.751*nm_per_Ang)*(2^(5/6)); % nm
    Param.Br.epsilon = Gamma*(0.0269586)*kJ_per_kcal; % kJ/mol

    Param.I.sigma = Beta*(2.919*nm_per_Ang)*(2^(5/6)); % nm
    Param.I.epsilon = Gamma*(0.0427845)*kJ_per_kcal; % kJ/mol

elseif strcmp(Watermodel,'TIP3P')
    %% JC Ion Parameters in TIP3P water
    Param.Li.sigma = Beta*(1.025*nm_per_Ang)*(2^(5/6)); % nm
    Param.Li.epsilon = Gamma*(0.0279896)*kJ_per_kcal; % kJ/mol

    Param.Na.sigma = Beta*(1.369*nm_per_Ang)*(2^(5/6)); % nm
    Param.Na.epsilon = Gamma*(0.0874393)*kJ_per_kcal; % kJ/mol

    Param.K.sigma = Beta*(1.705*nm_per_Ang)*(2^(5/6)); % nm
    Param.K.epsilon = Gamma*(0.1936829)*kJ_per_kcal; % kJ/mol

    Param.Rb.sigma = Beta*(1.813*nm_per_Ang)*(2^(5/6)); % nm
    Param.Rb.epsilon = Gamma*(0.3278219)*kJ_per_kcal; % kJ/mol

    Param.Cs.sigma = Beta*(1.976*nm_per_Ang)*(2^(5/6)); % nm
    Param.Cs.epsilon = Gamma*(0.4065394)*kJ_per_kcal; % kJ/mol

    Param.F.sigma = Beta*(2.303*nm_per_Ang)*(2^(5/6)); % nm
    Param.F.epsilon = Gamma*(0.0033640)*kJ_per_kcal; % kJ/mol

    Param.Cl.sigma = Beta*(2.513*nm_per_Ang)*(2^(5/6)); % nm
    Param.Cl.epsilon = Gamma*(0.0355910)*kJ_per_kcal; % kJ/mol

    Param.Br.sigma = Beta*(2.608*nm_per_Ang)*(2^(5/6)); % nm
    Param.Br.epsilon = Gamma*(0.0586554)*kJ_per_kcal; % kJ/mol

    Param.I.sigma = Beta*(2.860*nm_per_Ang)*(2^(5/6)); % nm
    Param.I.epsilon = Gamma*(0.0536816)*kJ_per_kcal; % kJ/mol

elseif strcmp(Watermodel,'TIP4PEW')
    %% JC Ion Parameters in TIP4P water
    Param.Li.sigma = Beta*(0.808*nm_per_Ang)*(2^(5/6)); % nm
    Param.Li.epsilon = Gamma*(0.1039884)*kJ_per_kcal; % kJ/mol

    Param.Na.sigma = Beta*(1.226*nm_per_Ang)*(2^(5/6)); % nm
    Param.Na.epsilon = Gamma*(0.1684375)*kJ_per_kcal; % kJ/mol

    Param.K.sigma = Beta*(1.590*nm_per_Ang)*(2^(5/6)); % nm
    Param.K.epsilon = Gamma*(0.2794651)*kJ_per_kcal; % kJ/mol

    Param.Rb.sigma = Beta*(1.709*nm_per_Ang)*(2^(5/6)); % nm
    Param.Rb.epsilon = Gamma*(0.4331494)*kJ_per_kcal; % kJ/mol

    Param.Cs.sigma = Beta*(1.888*nm_per_Ang)*(2^(5/6)); % nm
    Param.Cs.epsilon = Gamma*(0.3944318)*kJ_per_kcal; % kJ/mol

    Param.F.sigma = Beta*(2.538*nm_per_Ang)*(2^(5/6)); % nm
    Param.F.epsilon = Gamma*(0.0015752)*kJ_per_kcal; % kJ/mol

    Param.Cl.sigma = Beta*(2.760*nm_per_Ang)*(2^(5/6)); % nm
    Param.Cl.epsilon = Gamma*(0.0116615)*kJ_per_kcal; % kJ/mol

    Param.Br.sigma = Beta*(2.768*nm_per_Ang)*(2^(5/6)); % nm
    Param.Br.epsilon = Gamma*(0.0303773)*kJ_per_kcal; % kJ/mol

    Param.I.sigma = Beta*(2.952*nm_per_Ang)*(2^(5/6)); % nm
    Param.I.epsilon = Gamma*(0.0417082)*kJ_per_kcal; % kJ/mol
else
