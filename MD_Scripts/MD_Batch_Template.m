...
if strcmpi(Server,'sea') || strcmpi(Server,'pod') % Orcinus
    ...
    if Nodes > 1 && Settings.openMP
        mdrun = ['mpiexec -np ' num2str(Nodes*2) ...
            ' --npernode $MPI_PPN --mca mpi_paffinity_alone 0 ##MDRUN## -v -notunepme -ntomp $OMP_NUM_THREADS -maxh ' ...
            num2str(Settings.Hours)];
        gmx = 'gmx_mpi_d';
        MPI_PPN = 2;
        OMP_NUM_THREADS = 6;
    elseif Nodes > 1 && ~Settings.openMP
        mdrun = ['mpiexec -machinefile $PBS_NODEFILE -np ' num2str(Nodes*Cores_per_node) ...
            ' --npernode $MPI_PPN --mca mpi_paffinity_alone 0 ##MDRUN## -v -notunepme -ntomp $OMP_NUM_THREADS -maxh ' ...
            num2str(Settings.Hours)];
        gmx = 'gmx_mpi_d';
...
elseif strcmpi(Server,'bel') % Beluga
    ...
    TemplateText = ['#!/bin/bash' newline ... 
        ...
        'module load matlab/R2019a' newline ...
...
elseif ~isempty(regexp(Server,'se[0-9]','ONCE')) || strcmpi(Server,'log') % sockeye
    ...
    if Settings.openMP
        MPI_PPN = Cores_per_node/8;
        OMP_NUM_THREADS = Cores_per_node/4;
        mdrun = ['mpiexec -machinefile $PBS_NODEFILE -np ' num2str(MPI_PPN*Nodes) ...
            ' --map-by ppr:' num2str(MPI_PPN) ':node ##MDRUN## -v -ntomp $OMP_NUM_THREADS -maxh ' ...
            num2str(Settings.Hours)];
    else
        MPI_PPN = Cores_per_node;
        OMP_NUM_THREADS = 1;
        mdrun = ['mpiexec -machinefile $PBS_NODEFILE -np ' num2str(Total_Cores) ...
            ' --map-by ppr:' num2str(MPI_PPN) ':node ##MDRUN## -v -ntomp $OMP_NUM_THREADS -maxh ' ...
            num2str(Settings.Hours)];
    end
    gmx = 'gmx_mpi_d';
    ...
    TemplateText = ['#!/bin/bash' newline ...
        ...
        'select=' num2str(Nodes) ':ncpus=' num2str(Cores_per_node) ':mpiprocs=' num2str(MPI_PPN) ...
        ':ompthreads=' num2str(OMP_NUM_THREADS) ':mem=' memline newline ...
...
elseif strcmpi(Server,'ced') || strcmpi(Server,'cdr') || strcmpi(Server,'gra') % Cedar and graham
    ...
    TemplateText = ['#!/bin/bash' newline ... 
        ...
        newline newline...
        '# Set variables' newline ...
        'export OMP_NUM_THREADS=' OMP_NUM_THREADS newline ...
        newline newline ...
        '# Run Job' newline ...
        '##PREMIN##' newline ...
        '##EXT1##' newline ...
